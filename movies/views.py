...
from movies.forms import SearchForm, MovieNightForm, AttendanceForm, InviteeForm, MovieNightInvitaion

from movies.omdb_integration import search_and_save, fill_movie_details
from movies.models import Movie, MovieNight
...
def movie_night_detail(request, pk):
    ...
    if not is_creator:
        if request.user not in invitees:
            raise PermissionDenid("You do not have access to this MovieNight")
            ...
            if invitee_form.is_valid():
                ...
                if invitee == request.user or invitee in invitees:
                    invitee_form.add_error(
                        "email",
                        "That user is the creator or already invited."
                    )
                else:
                    MovieNightInvitaion.objects.create(
                        invitee=invitee,
                        movie_night=movie_night
                    )
                    return redirect(request.path) # effectively, just reload the page ^_0
        else:
            invitee_form = InviteeForm()
    
    return render(request, "movies/movie_night_detail.html", {
...
def movie_night_list(request):
    ...
    invited_movie_nights = MovieNight.objects.filter(
        start_time__gt=start_time_after,
        invites__in = MovieNightInvitaion.objects.filter(invitee=request.user)
    )

    return render(request, "movies/movie_night_list.html", {
        "page_group": "movie-nights",
